...
import {appProfile, toolkitItems} from '../../domain/berlinGuide';
import {useNavigation} from '../../app/navigation/NavigationContext';
import {colors} from '../../shared/theme';
import {openPhone, openWebsite} from '../../shared/native/deviceActions';

type Props = {
  toolId: string;
};

export function ToolkitBriefingView({toolId}: Props): React.JSX.Element {
  ...
  const runAction = () => {
    if (tool.action === 'arrival') {
      navigation.push('ArrivalPlanner');
      return;
    }
    if (tool.action === 'call') {
    ...
    if (tool.action === 'guide') {
      navigation.push('EntryPrimer');
      return;
    }
    if (tool.action === 'map') {
      navigation.push('BerlinMap');
      return;
    }
